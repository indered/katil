...
- Live Render health returned 404 after deploy -> the bootstrap script guessed the service-name URL instead of Render's slug URL -> re-read the service after deploy and use `serviceDetails.url` or `slug`.
